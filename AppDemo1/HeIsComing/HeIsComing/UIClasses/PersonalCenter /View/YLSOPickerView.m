...
-(void)layoutSubviews
{
    ...
    self.topView = [[UIView alloc]initWithFrame:YLSRect(0, 667/667, 1, 225/667+BOTTOM_HEIGHT)];
...
//    self.topView.layer.mask = maskLayer;
    ...
    UIButton *cancleButton = [UIButton buttonWithType:UIButtonTypeCustom];
      ...
      [cancleButton setTitleColor:Color666666 forState:UIControlStateNormal];
      [cancleButton setFrame:YLSRect(10/375, 5/667, 50/375, 40/667)];
      [cancleButton addTarget:self action:@selector(quit) forControlEvents:UIControlEventTouchUpInside];
      [self.topView addSubview:cancleButton];
    self.doneBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.doneBtn setTitle:@"完成" forState:UIControlStateNormal];
    [self.doneBtn setTitleColor:SaiColor(45, 133, 217) forState:UIControlStateNormal];
...
-(void)quit
{
    [UIView animateWithDuration:0.2 animations:^{
        ...
        if (!self.result) {
            if ([QKUITools isBlankArray:self.array]) {
                self.result=@"";
                return ;
            }
