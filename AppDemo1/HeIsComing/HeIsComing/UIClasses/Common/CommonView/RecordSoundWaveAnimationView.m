...
#define ALPHA 0.010f                 // 音频振幅调解相对值 (越小振幅就越高)
...
//    gradientLayer.colors = @[(id)[UIColor colorWithRed:37/255.0 green:166/255.0 blue:149/255.0 alpha:1].CGColor, (id)[UIColor colorWithRed:55/255.0 green:222/255.0 blue:107/255.0 alpha:1].CGColor, (id)[UIColor colorWithRed:37/255.0 green:166/255.0 blue:149/255.0 alpha:1].CGColor];
    gradientLayer.colors = @[(id)kColorFromRGBHex(0x4985FF).CGColor, (id)kColorFromRGBHex(0x4985FF).CGColor, (id)kColorFromRGBHex(0x4985FF).CGColor];
...
//    level = (level+37.5)*3.2 / self.levelLayer.frame.size.height;
    level = (level+100)*3.2 / self.levelLayer.frame.size.height;
...
//    self.currentLevelsArray = [NSMutableArray arrayWithArray:@[@(level*aa0),@(level*aa1),@(level*aa2),@(level*aa3),@(level*aa4),@(level*aa5),@(level*aa6),@(level*aa7),@(level*aa8),@(level*1),@(level*aa8),@(level*aa7),@(level*aa6),@(level*aa5),@(level*aa4),@(level*aa3),@(level*aa2),@(level*aa1),@(level*aa0),@(level*aa0),@(level*aa1),@(level*aa2),@(level*aa3),@(level*aa4),@(level*aa5),@(level*aa6),@(level*aa7),@(level*aa8),@(level*1),@(level*aa8),@(level*aa7),@(level*aa6),@(level*aa5),@(level*aa4),@(level*aa3),@(level*aa2),@(level*aa1),@(level*aa0)]];
     @autoreleasepool {

    NSMutableArray *currentLevelsArray=[NSMutableArray array];
    for (int i=0; i<_numberOfItems; i++) {
          
        if (i<10||i>40) {
                    [currentLevelsArray  addObject:[NSNumber numberWithDouble:(level*(3 + arc4random()%36/10.0 )/10.0)]];

        }else{
        [currentLevelsArray  addObject:[NSNumber numberWithDouble:(level*(4.0 + arc4random()%36/10.0 )/10.0)]];
        }
        
    }
    self.currentLevelsArray=currentLevelsArray;
         }

//    self.currentLevelsArray = [NSMutableArray arrayWithArray:@[@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),@(level*aa4),]];
