...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self.diskBgView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.height.mas_equalTo(ScreenWidth - 75.0f);
        }];
...
- (void)layoutSubviews {
    ...
    self.needleView.gk_centerX = ScreenWidth * 0.5 + 25.0f;
...
- (void)setCurrentIndex:(NSInteger)currentIndex needChange:(BOOL)needChange {
    if (currentIndex >= 0) {
        ...
        // 设置图片
        self.centerDiskView.imgUrl = centerM.pic_small;
        ...
        if (needChange) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                self.leftDiskView.imgUrl    = leftM.pic_small;
                self.rightDiskView.imgUrl   = rightM.pic_small;
