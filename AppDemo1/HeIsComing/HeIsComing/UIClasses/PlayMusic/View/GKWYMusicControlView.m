...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        CGFloat btnWH  = 50;
        CGFloat leftM  = 50;
        CGFloat margin = (ScreenWidth - 4 * btnWH - 2 * leftM) / 3;
        ...
        [self.listBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerY.equalTo(self.playBtn.mas_centerY);
            make.width.height.mas_offset(kSCRATIO(44));
        }];
//        [self.countdownLabel mas_makeConstraints:^(MASConstraintMaker *make) {
//                  make.centerX.equalTo(self.listBtn);
//                  make.top.equalTo(self.listBtn.mas_bottom).offset(kSCRATIO(5));
//              }];
    }
    return self;
}
...
- (void)setIs_Asmr:(BOOL)is_Asmr{
    _is_Asmr = is_Asmr;
    _countdownLabel.hidden=!is_Asmr;
    [_listBtn setImage:self.is_Asmr?[UIImage imageNamed:@"asmr_icon_timing"]:[UIImage imageNamed:@"cm2_icn_list"] forState:UIControlStateNormal];
          [_listBtn setImage:self.is_Asmr?[UIImage imageNamed:@"asmr_icon_timing"]:[UIImage imageNamed:@"cm2_icn_list_prs"] forState:UIControlStateHighlighted];
}
...
-(void)startCountdown:(NSInteger )second{
  __block  int i=second;
    [self stopCountdown];
   SaiContext.cyclesQueueItem= [[ZXCTimer shareInstance] addCycleTask:^{
       self.countdownLabel.text=[NSString stringWithFormat:@"%@",[QKUITools timeFormattedWithMMSS:(int)i--]];
       if (i<0) {
               [self stopCountdown];
...
- (UILabel *)countdownLabel{
    if (!_countdownLabel) {
        _countdownLabel=[UILabel CreatLabeltext:@"10:00" Font:[UIFont systemFontOfSize:kSCRATIO(9)] Textcolor:UIColor.whiteColor textAlignment:NSTextAlignmentCenter];
...
- (GKSliderView *)slider {
    if (!_slider) {
        ...
        [_slider setBackgroundImage:[UIImage imageNamed:@"cm2_fm_playbar_btn"] forState:UIControlStateDisabled];
        ...
        [_slider setThumbImage:[UIImage imageNamed:@"cm2_fm_playbar_btn_dot"] forState:UIControlStateDisabled];
