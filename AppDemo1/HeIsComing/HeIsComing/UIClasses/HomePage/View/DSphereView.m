...
- (void)setTagOfPoint: (DPoint)point andIndex:(NSInteger)index
{
    ...
    DHomeView *view = (DHomeView *)[tags objectAtIndex:index];
    
    view.layer.position=CGPointMake((point.x + 1) * (self.frame.size.width / 2.), (point.y + 1) * self.frame.size.width / 2.);
//        view.center = CGPointMake((point.x + 1) * (self.frame.size.width / 2.), (point.y + 1) * self.frame.size.width / 2.);
        CGFloat transform = (point.z + 2) / 2;
...
- (void)autoTurnRotation
{
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
               ...
               for (NSInteger i = 0; i < tags.count; i ++) {
            #pragma mark 常数angle角度
                    [self updateFrameOfPoint:i direction:normalDirection andAngle:0.001];
