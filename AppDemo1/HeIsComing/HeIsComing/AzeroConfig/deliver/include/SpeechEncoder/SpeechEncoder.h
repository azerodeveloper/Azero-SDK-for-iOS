...
private:
    ...
    void encodeLoop(
        ...
        avsCommon::avs::AudioInputStream::Reader::Reference reference);
    /*
     * Used when last encodeLoop not start and a new one is started (This
     * may happed when using stopEncoding(false) )
     */
    bool restartEncodeLoop(std::chrono::milliseconds timeout);

    /*
    *  @ref restartEncodeLoop wait for encode loop finish or timeout
    */
    std::mutex mutexCV;
    ...
    /// true when encode loop is active and only allowed changed in encode loop
    std::atomic<bool> m_encodeLoopActive;
