...
- (void)createUI{
    ...
    self.backgroundColor = kColorFromRGBA(26, 26, 26, 1);
...
//        [self.sendButton setBackgroundColor:[UIColor colorWithHex:@"#3d3d45"]];
    ...
    //---- 计算高度 ---- //
    CGSize size = CGSizeMake(WIDTH-65, CGFLOAT_MAX);
    ...
    if (curheight < 19.094) {
        statusTextView = NO;
        self.frame = CGRectMake(0, y - 49, WIDTH, 49);
    }else if(curheight < MaxTextViewHeight){
        statusTextView = NO;
        self.frame = CGRectMake(0, y - textView.contentSize.height-10, WIDTH,textView.contentSize.height+10);
    }else{
        statusTextView = YES;
        return;
    }
...
#pragma mark-- 注释了
//    self.frame = CGRectMake(0, kScreenHeight - 49, WIDTH, 49);

    CGFloat y = self.selfFrame.origin.y;
    self.frame = CGRectMake(0, y, WIDTH, 49);
}
