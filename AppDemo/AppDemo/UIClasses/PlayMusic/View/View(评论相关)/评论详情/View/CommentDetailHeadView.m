...
-(instancetype)init
{
    ...
    if (self) {
        ...
        //昵称
        self.userNameLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.avatarIV.frame)+kGAP, k750AdaptationWidth(30), 200, 20)];
...
//        [_likeBtn layoutButtonWithEdgeInsetsStyle:ButtonEdgeInsetsStyleImageLeft imageTitlespace:4];
        _likeBtn.frame = CGRectMake(WIDTH - k750AdaptationWidth(100) - k750AdaptationWidth(30), k750AdaptationWidth(30), k750AdaptationWidth(100), kAvatar_Size);
        ...
        self.timeLable.frame = CGRectMake(CGRectGetMaxX(self.avatarIV.frame)+kGAP, CGRectGetMaxY(self.userNameLabel.frame), 200, 20);
        
        
        _line = [[UIImageView alloc]init];
        _line.backgroundColor = kColorFromRGBA(255, 255, 255, 0.2);
...
-(void)setMessageModel:(MessageInfoModel *)messageModel
{
    ...
    CGFloat maxW = WIDTH - ( k750AdaptationWidth(30) + kGAP + kAvatar_Size + k750AdaptationWidth(40));
    ...
    self.line.frame = CGRectMake(0, CGRectGetMaxY(self.messageTextLabel.frame) + space, WIDTH, k750AdaptationWidth(1));
    ...
    if (messageModel.replyList.count >0) {
        self.commnetsNumber.text = [NSString stringWithFormat:@"回复（%lu）",(unsigned long)messageModel.replyList.count];
    }else{
        self.commnetsNumber.text = @"暂无评论";
    }
    
    self.frame = CGRectMake(0, 0, WIDTH, CGRectGetMaxY(self.commnetsNumber.frame));
}
