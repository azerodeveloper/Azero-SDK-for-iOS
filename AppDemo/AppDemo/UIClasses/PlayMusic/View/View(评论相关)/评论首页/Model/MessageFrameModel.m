...
-(void)setModel:(MessageInfoModel *)model
{
    ...
    //名称
    self.nameFrame = CGRectMake(CGRectGetMaxX(self.iconFrame)+kGAP, k750AdaptationWidth(30),WIDTH-kAvatar_Size-2*kGAP-kGAP, 20);
    
    //点赞
    self.lickBtnFrame = CGRectMake(WIDTH - k750AdaptationWidth(100) - k750AdaptationWidth(30), k750AdaptationWidth(30), k750AdaptationWidth(100), kAvatar_Size);
...
//    self.timeFrame = CGRectMake(CGRectGetMaxX(self.iconFrame)+kGAP, CGRectGetMaxY(self.nameFrame), timeSize.width, Comment_time_H);
    ...
    YYTextLayout *layout = [YYTextLayout layoutWithContainerSize:CGSizeMake(WIDTH-kGAP-kAvatar_Size-2*kGAP, CGFLOAT_MAX) text:attrString];
    CGFloat introHeight = layout.textBoundingSize.height;
    self.bigCommentFrame = CGRectMake(kGAP+kAvatar_Size+kGAP,CGRectGetMaxY(self.timeFrame) + kGAP, WIDTH - 2 * kGAP-kAvatar_Size-kGAP, introHeight); //这个是评大评论的高度
    ...
    for (CommentInfoModel *commentModel in model.replyList) {
        ...
        CGSize maxSize = CGSizeMake(WIDTH - 2*kGAP-kAvatar_Size-10, CGFLOAT_MAX);
