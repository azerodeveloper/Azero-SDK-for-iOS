...
//        }];
        [self.playButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self).offset(kSCRATIO(15));
            make.top.mas_offset(kSCRATIO(130));
            make.width.height.mas_offset(kSCRATIO(15));
        }];
        [lineLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.playButton.mas_right).offset(kSCRATIO(10));
            make.centerY.equalTo(self.playButton);
            make.width.mas_offset(kSCRATIO(23));
            make.height.mas_offset(kSCRATIO(1));

        }];
        [lineLabel layoutIfNeeded];
        lineLabel.backgroundColor=[UIColor colorWithPatternImage:[UIImage imageWithGradualChangingColor:^(QQGradualChangingColor *graColor) {
            graColor.toColor =kColorFromARGBHex(0x67E4FD, 0) ;
            graColor.fromColor = Color67E4FD;
            graColor.type = QQGradualChangeTypeLeftToRight;
        } size:lineLabel.bounds.size cornerRadius:QQRadiusMakeSame(0)]];
...
- (void)setLyrics:(NSArray *)lyrics {
    ...
    if (lyrics) {
        if (lyrics.count == 0) {
            ...
            // 滚动到中间行
            NSIndexPath *indexPath = [NSIndexPath indexPathForRow:lyricIndexAppend/2 inSection:0];
            [self.lyricTable scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionTop animated:NO];
    ...
    for (NSInteger i = 0; i < self.lyrics.count; i++) {
        ...
        if ((self.lyricIndex != i && currentTime > currentLyric.msTime) && (!nextLyric || currentTime < nextLyric.msTime)) {
            ...
            if (!self.isScrolling) {
                ...
                NSIndexPath *indexPath = [NSIndexPath indexPathForRow:(self.lyricIndex + lyricIndexAppend/2) inSection:0];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    // 多加10行，是为了上下留白
    return self.lyrics.count + lyricIndexAppend;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"LyricCell" forIndexPath:indexPath];
    cell.textLabel.textColor     = [UIColor colorWithWhite:1 alpha:0.5];
    cell.textLabel.textAlignment = NSTextAlignmentCenter;
    cell.textLabel.numberOfLines = 0;
    cell.textLabel.font          = [UIFont systemFontOfSize:kSCRATIO(17)];
    cell.selectedBackgroundView  = [UIView new];
    cell.backgroundColor         = [UIColor clearColor];
    
    if (indexPath.row < lyricIndexAppend/2 || indexPath.row > self.lyrics.count + lyricIndexAppend/2-1) {
        cell.textLabel.textColor = [UIColor clearColor];
        cell.textLabel.text      = @"";
    }else {
        cell.textLabel.text = [self.lyrics[indexPath.row - lyricIndexAppend/2] content];
        
        if (indexPath.row == self.lyricIndex + lyricIndexAppend/2) {
            cell.textLabel.textColor = kColorFromRGBHex(0x081B02);
            cell.textLabel.font      = [UIFont systemFontOfSize:kSCRATIO(18)];
        }else {
            cell.textLabel.textColor = kColorFromRGBHex(0x1E3317);
            cell.textLabel.font      = [UIFont systemFontOfSize:kSCRATIO(16)];
        }
    }
...
-(UIButton *)playButton{
    if (!_playButton) {
        ...
        [_playButton setImage:[UIImage imageNamed:@"playSmall_icon"] forState:0];
        [_playButton setImage:[UIImage imageNamed:@"pauseSmall_icon"] forState:UIControlStateSelected];
