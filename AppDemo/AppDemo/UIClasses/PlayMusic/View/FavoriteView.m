...
- (instancetype)init {
    return [self initWithFrame:CGRectMake(0, 0, kSCRATIO(38),  kSCRATIO(34))];
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if(self) {
        ...
        _favoriteBefore.image = [UIImage imageNamed:@"nolike"];
        ...
        _favoriteAfter.image = [UIImage imageNamed:@"isLike"];
...
- (void)handleGesture:(UITapGestureRecognizer *)sender {
    switch (sender.view.tag) {
        case kFavoriteViewLikeBeforeTag: {
            ...
            if (self.isFavoriteBlock) {
                self.isFavoriteBlock(YES);
            }
            break;
        }
        case kFavoriteViewLikeAfterTag: {
            [self startLikeAnim:NO];
            self.isFavoriteBlock(NO);
