...
+ (NSArray *)lyricParserWithUrl:(NSString *)url isDelBlank:(BOOL)isDelBlank {
    ...
    if ([url hasPrefix:@"http"] || [url hasPrefix:@"https"]) { // 网络路径
        ...
        NSString *lyricStr = [NSString stringWithContentsOfURL:[NSURL URLWithString:url] encoding:NSUTF8StringEncoding  error:&error];
      ...
      {
          lyricStr = [NSString stringWithContentsOfURL:[NSURL URLWithString:url] encoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000) error:&error];
        ...
        TYLog(@"歌词的error是  %@",error);
        return [self lyricParaseWithLyricString:lyricStr isDelBlank:isDelBlank];
    }else { // 本地路径
        NSString *lyricStr = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"674076063" ofType:@"lrc"] encoding:NSUTF8StringEncoding error:nil];
        return [self lyricParaseWithLyricString:lyricStr isDelBlank:isDelBlank];
    }
}
