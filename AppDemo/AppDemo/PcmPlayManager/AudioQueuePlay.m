...
//    TYLog(@"%@",data);
//
//    TYLog(@"%@",[[OpusCodec shared]encode:data]);
//    NSData *data2=[[OpusCodec shared]encode:data];
//    TYLog(@"%@",[[OpusCodec shared]decode:data2]);
//    data=[[OpusCodec shared]decode:data2];
    if (self.stop) {
        return;
    }
...
static void listenerCallback(
    ...
    OSStatus err = AudioQueueGetProperty (inAQ, kAudioQueueProperty_IsRunning, &isRunning, &size);
       ...
       if (isRunning) {
           [AudioQueuePlay sharedAudioQueuePlay].isSpeaking=YES;
       } else {
           TYLog(@"listenerCallback  --- stop");
           [AudioQueuePlay sharedAudioQueuePlay].isSpeaking=NO;

           if (!player.isInterrupted) {
                      [[NSNotificationCenter defaultCenter] postNotificationName:SaiTtsPlayComplete object:nil];
