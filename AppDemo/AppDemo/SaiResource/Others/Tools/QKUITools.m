...
+(NSString *)getNowyyyymmddmmss{
    NSDate *now = [NSDate date];
    NSDateFormatter *formatDay = [[NSDateFormatter alloc] init];
    formatDay.dateFormat = @"HH:mm:ss.SSSSSS";
    NSString *dayStr = [formatDay stringFromDate:now];
...
+(BOOL)isSaiHeadsetPluggedIn{
    ...
    BOOL headphonesLocated = NO;
    for (AVAudioSessionPortDescription *outputPortDesc in currentRount.outputs) {
        if ([outputPortDesc.portName isEqualToString:@"M18 Pro"]||[outputPortDesc.portName isEqualToString:@"SoundAI TA"]||[outputPortDesc.portName isEqualToString:@"SoundAI TA Pro"]) {
            headphonesLocated = YES;
        }
...
+(BOOL)isSaiHeadsetNeedBle{  
    ...
    BOOL headphonesLocated = NO;
    for (AVAudioSessionPortDescription *outputPortDesc in currentRount.outputs) {
//        if ([outputPortDesc.portName isEqualToString:@"SoundAI TA Pro"]) {

        if ([outputPortDesc.portName isEqualToString:@"SoundAI TA Pro"]||[outputPortDesc.portName isEqualToString:@"SoundAI TA Buds"]) {
            headphonesLocated = YES;
        }
