...
- (void)detectAndSetSerFile{
    if ([QKUITools isSaiHeadsetPluggedIn]) {//如果是声智的耳机
        ...
        if (fileRet == NO) {
            [MessageAlertView showHudMessage:@"声智ser写文件失败"];
        }
    }else{
        BOOL fileRet = [[SaiAzeroManager sharedAzeroManager] saiOriginalSerSet];
        if (fileRet == NO) {
            [MessageAlertView showHudMessage:@"原始ser写文件失败"];
        }
    }
    //重新启动opdenoise
    BOOL reset = [[SaiAzeroManager sharedAzeroManager] saiRetSerFile];
    if (reset == NO) {
        [MessageAlertView showHudMessage:@"重启openDenoise失败"];
    }
}
//上传日志
...
- (void)saiAzeroManagerAction:(NSString * )action andPosition:(NSString * )position  andResourceId:(NSString * )resourceId{
    [self saiAzeroManagerAction:action andPosition:position selectedPosition:@"" andResourceId:resourceId andkeyword:@""];
}
- (void)saiAzeroManagerAction:(NSString * )action andPosition:(NSString * )position  andResourceId:(NSString * )resourceId acquireType:(NSString *)acquireType{
    self.eventString=[SaiTextInputModel azeroModelAction:action position:position selectedPosition:@""  andResourceId:resourceId andkeyword:@"" acquireType:acquireType];
    NSLog(@"播放指令：%@",self.eventString);
    [self.myAzeroExpress sendEvent: self.eventString];
...
- (void)saiAzeroManagerAction:(NSString * )action andPosition:(NSString * )position selectedPosition:(NSString * )selectedPosition andResourceId:(NSString * )resourceId andkeyword:(NSString * )keyword{
    self.eventString=[SaiTextInputModel azeroModelAction:action position:position selectedPosition:selectedPosition  andResourceId:resourceId andkeyword:keyword];
    
    [self.myAzeroExpress sendEvent: self.eventString];
//    [self retransmissionMechanism];
}
-(void)retransmissionMechanism{
   self.cyclesQueueItem=nil;
   self.cyclesQueueItem  = [[ZXCTimer shareInstance]addCycleTask:^{
        [self.myAzeroExpress sendEvent: self.eventString];

    } timeInterval:10 runCount:2];
    
}
-(void)removeRetransmissionMechanism{
    [[ZXCTimer shareInstance] removeCycleTask:self.cyclesQueueItem];
