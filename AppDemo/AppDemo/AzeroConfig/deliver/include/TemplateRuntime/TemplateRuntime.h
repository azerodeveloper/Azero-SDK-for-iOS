...
private:
    ...
    void handleClearTemplateDirective(std::shared_ptr<DirectiveInfo> info);

    void handleClearPlayerInfoDirective(std::shared_ptr<DirectiveInfo> info);
    /**
    ...
    void executeStartPlayerTimer(std::chrono::milliseconds timeout);
    ...
    class PlayerTemplateRuntime
        : public avsCommon::sdkInterfaces::ChannelObserverInterface
        , public std::enable_shared_from_this<PlayerTemplateRuntime>{
    public:
        TemplateRuntime *parent;
        State m_state;
        avsCommon::avs::FocusState m_focus;

        PlayerTemplateRuntime(TemplateRuntime *templateRuntime)
        :
        parent(templateRuntime),
        m_state{TemplateRuntime::State::IDLE},
        m_focus{avsCommon::avs::FocusState::NONE}{}

    /// Timer that is responsible for clearing the display.
        avsCommon::utils::timing::Timer m_clearDisplayTimer;
        void executeClearPlayerCard();
        void executePlayerTimerEvent();
        void executeOnFocusChangedEvent(avsCommon::avs::FocusState newFocus);
        void executeDisplayCard();
        void executeStopTimer();
        /**
     ...
     * Destructor.
     */
    virtual ~PlayerTemplateRuntime() = default;
    };

    std::shared_ptr<PlayerTemplateRuntime> m_PlayerTemplateRuntime;
};
