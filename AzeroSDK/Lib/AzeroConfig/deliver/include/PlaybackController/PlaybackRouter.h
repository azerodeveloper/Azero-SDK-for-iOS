...
public:
    ...
    virtual void setLocalHandlerForAudioPlayer(std::shared_ptr<avsCommon::sdkInterfaces::PlaybackHandlerInterface> handler) override;
    /// @}
...
private:
    ...
    /// The additional handler for audio player
    std::shared_ptr<avsCommon::sdkInterfaces::PlaybackHandlerInterface> m_loaclHandlerForAudioPlayer;

    /// Mutex protecting the observer pointer.
    std::mutex m_handlerMutex;

    std::mutex m_localHandlerForAudioPlayerMutex;
};
