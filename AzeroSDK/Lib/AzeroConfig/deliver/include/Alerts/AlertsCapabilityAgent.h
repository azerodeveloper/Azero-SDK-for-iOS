...
static const std::chrono::minutes ALERT_PAST_DUE_CUTOFF_MINUTES = std::chrono::minutes(10);  // 30
...
public:
    ...
    /**
     * This function provides a way for application code to request this object remove alert as the result
     * of a user action, such as pressing a physical button and input alertToken on the device.
     */
    void onRemoveAlert(const std::string& alertToken);
...
private:
    ...
    /**
     * A callback function to notify an object that an alert has been created with additional information about the
     * alert.
     *
     ...
     */
    void executeOnAlertCreated(const std::string& alertToken, const std::string& jsonPayload);
     ...
     */
    void executeOnAlertDeleted(const std::string& alertToken);
     ...
     * @param alertToken The alert token. See AlertObserverInterface for more.
     */
    void executeRemoveAlert(const std::string& alertToken);

    /**
     * A handler function which will be called by our internal executor to remove all alerts currently being
     * managed.
     */
    ...
    /**
     * Utility function to send a local Event to AVS. If isCertified is set to true, then the Event
     ...
     * @param payload The message payload of the Alert being sent to AVS within the Event.
     * @param isCertified Whether the event must be guaranteed to be sent.  See function description for details.
     */
    void sendLocalEvent(const std::string& eventName, const rapidjson::Document& payload, bool isCertified = false);
