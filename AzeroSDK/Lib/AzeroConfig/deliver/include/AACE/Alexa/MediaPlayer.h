...
public:
    ...
    virtual bool prepare( const std::string& url ) = 0;
     ...
     /**
     * Notifies the platform implementation to prepare for playback of an audio stream source with a certain audio format.
     * Audio data will be available to stream from the Engine via @c read(). After returning @c true, the Engine will call
     * @c play() to initiate audio playback.
     *
     * @return @c true if the platform implementation successfully handled the call, 
     * else @c false
     */
    virtual bool prepare(const alexaClientSDK::avsCommon::utils::AudioFormat* format){
        return false;
    } ;
