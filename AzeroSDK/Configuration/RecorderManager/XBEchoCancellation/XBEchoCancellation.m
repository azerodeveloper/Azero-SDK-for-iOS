...
@property (nonatomic ,copy) XBEchoCancellation_RecordCallBackBlock XBEchoCancellation_RecordCallBackHandle;
...
-(void)createAUGraphNoEchoCancellation:(MyAUGraphStruct*)augStruct{
    //Create graph
    CheckError(NewAUGraph(&augStruct->graph),
               "NewAUGraph failed");
...
//        [[SaiAzeroManager sharedAzeroManager] saiUpdateMessageWithLevel:SaiINFO tag:nil messmage:@"827record XBEchoCancellation **************** if (echoCancellation.isNeedInputCallback == NO) 354"];
    ...
    NSDictionary *dict = @{@"data":recordData,@"volLDB":@"0"};
        ...
        if (self.XBEchoCancellation_RecordCallBackHandle) {
            self.XBEchoCancellation_RecordCallBackHandle(dict);
        }
...
- (void)headsetModelNoEchoCancellation{
    [self stop];
//    [self setupSession];
    [self createAUGraphNoEchoCancellation:&myStruct];
    [self setupRemoteIOUnit:&myStruct];
    [self startGraph:myStruct.graph];
//    [self closeEchoCancellation];
    CheckError(AudioOutputUnitStart(myStruct.remoteIOUnit), "AudioOutputUnitStart failed");
}
- (void)normalModeEchoCancellation{
    [self stop];
//    [self setupSession];
    [self createAUGraph:&myStruct];
    [self setupRemoteIOUnit:&myStruct];
...
- (void)echoCancellation_RecordCallBackBlock:(XBEchoCancellation_RecordCallBackBlock)recordCallBackBlock{
    if (recordCallBackBlock) {
        self.XBEchoCancellation_RecordCallBackHandle = recordCallBackBlock;
    }
}
@end
